...
# Peek
def test_peek_returns_front_element_without_removing_it(queue):
    # Arrange
    queue.enqueue(10)
    queue.enqueue(15)
    ...
    # Assert
    assert front_element == 10
    assert str(queue) == '10 -> 15'
    assert queue.size == 2


def test_peek_returns_none_for_empty_list(queue):
    # Act
    front_element = queue.peek()

    # Assert
    assert not front_element
